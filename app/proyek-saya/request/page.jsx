...
export default function RequestMasukPage() {
  ...
  // State untuk menangani projectId dari URL
  ...
  // Ambil projectId dari URL saat komponen dimount
  useEffect(() => {
  ...
  useEffect(() => {
    const loadRequests = async () => {
      setLoading(true);
      try {
        const data = await RequestAPI.getIncomingRequests(urlProjectId || null);
        setRequests(data || []);
      } catch (err) {
    ...
    if (urlProjectId !== null) { // Tunggu sampai urlProjectId selesai di-set
      loadRequests();
    }
  }, [urlProjectId]);
  ...
  useEffect(() => {
    const loadProjectTitle = async () => {
      ...
      try {
        const p = await ProjectAPI.getMyProjectById(urlProjectId);
          ...
          <div className="flex items-center gap-2 text-sm text-gray-400 mb-6">
            ...
            {projectTitle && (
              <>
                ...
                <span
                  className="hover:text-[#004A74] cursor-pointer"
                  onClick={() => navigateWithQuery("/proyek-saya/detail", { id: urlProjectId })}
                >
                  {projectTitle}
                </span>
              </>
              ...
              {filteredRequests.map((req) => {
                ...
                return (
                  <div
                    ...
                    onClick={() => navigateWithQuery("/proyek-saya/request/detail", { id: req.id })}
                  >
                    <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                      ...
                      <div className="flex flex-col gap-2">
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            navigateWithQuery("/proyek-saya/request/detail", { id: req.id });
                          }}
