...
export default function RequestMasukPage() {
  ...
  const pathname = usePathname(); // contoh: /proyek-saya/request/123
  const projectId = pathname.split("/").pop(); // ambil projectId terakhir
  ...
  useEffect(() => {
    ...
    const loadRequests = async () => {
      setLoading(true);
      try {
        const data = await RequestAPI.getIncomingRequests(projectId);
        setRequests(data || []);
      } catch (err) {
  ...
  const handleApprove = async (id) => {
    try {
      ...
      setRequests((prev) => 
        prev.map((r) => r.id === id ? { ...r, status: "Approved", approved: true } : r)
      );
      if (updated.proposalLink) alert(`Request berhasil disetujui! Link proposal: ${updated.proposalLink}`);
      else alert('Request berhasil disetujui!');
    } catch (err) {
            ...
            <div className="flex items-center gap-2 text-sm text-gray-400 mb-6">
              ...
              {projectTitle && (
                <>
                  ...
                  <span
                    className="hover:text-[#004A74] cursor-pointer"
                    onClick={() => router.push(`/proyek-saya/detail/${projectId}`)}
                  >
                    {projectTitle}
                  </span>
                </>
                ...
                {filteredRequests.map((req) => {
                  ...
                  return (
                    <div
                      ...
                      onClick={() => router.push(`/proyek-saya/request/detail/${req.id}`)}
                    >
                      <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                        ...
                        <div className="flex flex-col gap-2">
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              router.push(`/proyek-saya/request/detail/${req.id}`);
                            }}
